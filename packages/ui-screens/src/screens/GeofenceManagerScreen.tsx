...
export const GeofenceManagerScreen: React.FC<GeofenceManagerScreenProps> = ({
  geofences = [
    ...
    setZones((prev) =>
      prev.map((z) => {
        if (z.id === id) {
          ...
          if (onGeofenceToggle) onGeofenceToggle(id, nextStatus);
  ...
  return (
    <div style={{ padding: "16px", backgroundColor: "#0f172a", color: "#f8fafc", borderRadius: "12px", fontFamily: "sans-serif" }}>
      <h2 style={{ margin: "0 0 16px 0", fontSize: "1.25rem", color: "#38bdf8" }}>Geofence Policy & Zone Manager</h2>
      <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
        {zones.map((zone) => (
          ...
          >
            <div>
              <div style={{ fontWeight: "bold", fontSize: "0.95rem", color: "#f1f5f9" }}>{zone.name}</div>
              <div style={{ fontSize: "0.8rem", color: "#94a3b8", marginTop: "2px" }}>
                Type: {zone.type} {zone.radiusMeters ? `(${zone.radiusMeters}m)` : ""}
              </div>
            </div>
            <button
