 ...
 */
import { GeoSphereRealtimeSDK, GeoSphereMockRealtimeProvider, GeoSphereRealtimeError, REALTIME_UI_COMPONENTS, REALTIME_READY_MADE_SCREENS, resolveThemePrecedence, validateLocaleConfig, evaluatePermissionState } from "../contracts/index.js";
...
async function runRealtimeSdkTests() {
    ...
    if (!Array.isArray(replayEvents))
        throw new Error("replay() failed to return event array");
    ...
    const tenantAChannel = "tenant_company_a";
    const tenantBChannel = "tenant_company_b";
    if (tenantAChannel === tenantBChannel)
        throw new Error("Tenant boundary error");
    ...
    if (trackEnv.eventType !== "tracking.locationUpdated")
        throw new Error("Tracking SDK event type check failed");
    ...
    if (gfEnv.eventType !== "geofence.entered")
        throw new Error("Geofencing SDK event type check failed");
    ...
    if (navEnv.eventType !== "navigation.maneuverChanged")
        throw new Error("Navigation SDK event type check failed");
    ...
    if (!theme.colors.primary)
        throw new Error("Theme resolution failed for Realtime SDK");
    ...
    if (connectState.status !== "granted")
    ...
    if (pubState.status !== "denied" || pubState.mode !== "disable")
    ...
    if (typeof GeoSphereRealtimeSDK.prototype.createPolygon === "function") {
    ...
    if (typeof GeoSphereRealtimeSDK.prototype.renderMap === "function") {
    ...
    if (typeof GeoSphereRealtimeSDK.prototype.watchPosition === "function") {
    ...
    if (typeof GeoSphereRealtimeSDK.prototype.startTrackingSession === "function") {
    ...
    if (typeof GeoSphereRealtimeSDK.prototype.evaluateGeofence === "function") {
    ...
    if (typeof GeoSphereRealtimeSDK.prototype.calculateRoute === "function") {
    ...
    if (typeof GeoSphereRealtimeSDK.prototype.startTurnByTurn === "function") {
    ...
    if (typeof GeoSphereRealtimeSDK.prototype.searchPlaces === "function") {
    ...
    if (typeof GeoSphereRealtimeSDK.prototype.calculateArea === "function") {
    ...
    if (typeof GeoSphereRealtimeSDK.prototype.createMapPackage === "function") {
    ...
    if (mockHealth.latencyMs !== 15)
        throw new Error("Mock provider health latency check failed");
    ...
    try {
        ...
        if (!(e instanceof GeoSphereRealtimeError) || e.code !== "CONNECTION_FAILED")
            throw e;
    }
    ...
    if (config.heartbeatIntervalMs !== 15000)
        throw new Error("heartbeatIntervalMs config check failed");
    console.log(" [42/44] Testing Auto Reconnect Config Flag...");
    if (!config.autoReconnect)
        throw new Error("autoReconnect config check failed");
    console.log(" [43/44] Testing Max Reconnect Retries Config Flag...");
    if (config.maxReconnectRetries !== 5)
        throw new Error("maxReconnectRetries config check failed");
...
//# sourceMappingURL=realtime-sdk.test.js.map