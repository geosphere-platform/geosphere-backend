 ...
 */
import { GeoSphereSpatialAnalysisSDK, GeoSphereMockSpatialAnalysisProvider, GeoSphereSpatialAnalysisError, SPATIAL_ANALYSIS_UI_COMPONENTS, SPATIAL_ANALYSIS_READY_MADE_SCREENS, resolveThemePrecedence, validateLocaleConfig, evaluatePermissionState } from "../contracts/index.js";
...
async function runSpatialAnalysisSdkTests() {
    ...
    const ptA = { type: "Point", coordinates: [79.0882, 21.1458] }; // SF / Nagpur pt 1
    const ptB = { type: "Point", coordinates: [79.0472, 21.0922] }; // pt 2
    ...
    if (!theme.colors.primary)
        throw new Error("Theme resolution failed for Spatial Analysis SDK");
    ...
    if (measureState.status !== "granted")
    ...
    if (queryState.status !== "denied" || queryState.mode !== "disable")
    ...
    const hasRoute = typeof GeoSphereSpatialAnalysisSDK.prototype.calculateRoute === "function";
    if (hasRoute)
        throw new Error("DUPLICATION VIOLATION: Spatial Analysis SDK re-implemented Routing Engine!");
    // 34. Zero Duplicate Search / Location Engine Audit
    console.log(" [34/35] Verifying Zero Duplicate Search / Location Engine in Core...");
    const hasSearch = typeof GeoSphereSpatialAnalysisSDK.prototype.searchPlaces === "function";
    if (hasSearch)
        throw new Error("DUPLICATION VIOLATION: Spatial Analysis SDK re-implemented Search Engine!");
    // 35. Error Normalization Check
    console.log(" [35/35] Testing Spatial Analysis Error Normalization...");
    try {
        await sdk.calculateDistance(null, ptB);
        throw new Error("calculateDistance failed to throw on null geometry");
    }
    catch (e) {
        if (!(e instanceof GeoSphereSpatialAnalysisError) || e.code !== "INVALID_GEOMETRY")
            throw e;
    }
...
//# sourceMappingURL=spatial-analysis-sdk.test.js.map