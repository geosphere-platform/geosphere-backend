 ...
 */
import { GeoSphereLocationSDK, validateGeoSphereLocation, classifyLocationAccuracy, isLocationStale, LOCATION_UI_COMPONENTS, LOCATION_READY_MADE_SCREENS, resolveThemePrecedence, validateLocaleConfig, evaluatePermissionState } from "../contracts/index.js";
...
async function runLocationSdkTests() {
    ...
    try {
        ...
        if (!e.message.includes("INVALID_LOCATION"))
            throw e;
    }
    // 3. Accuracy Classification & Quality Levels
    console.log("  [3/20] Testing Accuracy Classification & Quality Levels...");
    if (classifyLocationAccuracy(3) !== "VERY_HIGH")
        throw new Error("Accuracy classification failed for 3m");
    if (classifyLocationAccuracy(10) !== "HIGH")
        throw new Error("Accuracy classification failed for 10m");
    if (classifyLocationAccuracy(30) !== "MEDIUM")
        throw new Error("Accuracy classification failed for 30m");
    if (classifyLocationAccuracy(100) !== "LOW")
        throw new Error("Accuracy classification failed for 100m");
    if (classifyLocationAccuracy(null) !== "UNKNOWN")
        throw new Error("Accuracy classification failed for null");
    ...
    if (typeof locationSdk.render === "function" || typeof locationSdk.componentDidMount === "function") {
...
//# sourceMappingURL=location-sdk.test.js.map