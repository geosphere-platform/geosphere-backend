 ...
 */
import { GeoSphereSchedulingSDK, GeoSphereMockSchedulingProvider, GeoSphereSchedulingError, SCHEDULING_UI_COMPONENTS, SCHEDULING_READY_MADE_SCREENS, resolveThemePrecedence, validateLocaleConfig, evaluatePermissionState } from "../contracts/index.js";
...
async function runSchedulingSdkTests() {
    ...
    const conflicts = await sdk.detectConflicts(newCal.calendarId, newEvt.startAt, newEvt.endAt, ["asset_seed_001"]);
    ...
    if (!theme.colors.primary)
        throw new Error("Theme resolution failed for Scheduling SDK");
    ...
    if (validLocale.timeFormat !== "HH:mm")
    ...
    if (execState.status !== "denied" || execState.mode !== "disable")
    ...
    if (typeof GeoSphereSchedulingSDK.prototype.createPolygon === "function") {
    ...
    if (typeof GeoSphereSchedulingSDK.prototype.renderMap === "function") {
    ...
    if (typeof GeoSphereSchedulingSDK.prototype.watchPosition === "function") {
    ...
    if (typeof GeoSphereSchedulingSDK.prototype.startTrackingSession === "function") {
    ...
    if (typeof GeoSphereSchedulingSDK.prototype.evaluateGeofence === "function") {
    ...
    if (typeof GeoSphereSchedulingSDK.prototype.calculateRoute === "function") {
    ...
    if (typeof GeoSphereSchedulingSDK.prototype.startTurnByTurn === "function") {
    ...
    if (typeof GeoSphereSchedulingSDK.prototype.searchPlaces === "function") {
    ...
    if (typeof GeoSphereSchedulingSDK.prototype.calculateArea === "function") {
    ...
    if (typeof GeoSphereSchedulingSDK.prototype.createMapPackage === "function") {
    ...
    if (typeof GeoSphereSchedulingSDK.prototype.createDriverShift === "function") {
    ...
    if (typeof GeoSphereSchedulingSDK.prototype.markAttendance === "function") {
    ...
    if (typeof GeoSphereSchedulingSDK.prototype.dispatchFleet === "function") {
    ...
    if (!Array.isArray(calEvents) || calEvents.length === 0)
        throw new Error("listEvents filter by calendarId failed");
    ...
    try {
        ...
        if (!(e instanceof GeoSphereSchedulingError) || e.code !== "CALENDAR_NOT_FOUND")
            throw e;
    }
    console.log(" [44/46] Testing Multi-Platform Adapter Metadata for Screens...");
    if (!calScreen.adapters || calScreen.adapters.length !== 3)
        throw new Error("calScreen adapters check failed");
    console.log(" [45/46] Testing Bounded Recurrence Frequency Validation...");
    const recRule = { frequency: "WEEKLY", interval: 1, count: 10 };
    if (recRule.frequency !== "WEEKLY" || recRule.count !== 10)
        throw new Error("Recurrence rule check failed");
    console.log(" [46/46] Testing Multi-Tenant Boundary Protection...");
    const tenantACal = "cal_tenant_A";
    const tenantBCal = "cal_tenant_B";
    if (tenantACal === tenantBCal)
...
//# sourceMappingURL=scheduling-sdk.test.js.map