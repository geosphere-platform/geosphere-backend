...
import {
  GeoSphereOfflineSDK,
  GeoSphereOfflineConfig,
  GeoSphereInMemoryOfflineStorage,
  GeoSphereOfflineError,
  OFFLINE_UI_COMPONENTS,
  OFFLINE_READY_MADE_SCREENS,
...
async function runOfflineSdkTests() {
  ...
  const config: GeoSphereOfflineConfig = {
  ...
  const bounds: [number, number, number, number] = [79.0, 21.0, 79.2, 21.2]; // Nagpur region
  ...
  sdk.subscribe((evt) => {
    if (evt.type.startsWith("offline.")) progressEvents.push(evt);
  });
  ...
  if (routePkg.type !== "ROUTING_DATA") throw new Error("Offline routing package designation failed");
  ...
  if (searchPkg.type !== "SEARCH_DATA") throw new Error("Offline search dataset package designation failed");
  ...
  if (!theme.colors.primary) throw new Error("Theme resolution failed for Offline SDK");
  ...
  if (typeof (GeoSphereOfflineSDK.prototype as any).createPolygon === "function") {
  ...
  if (typeof (GeoSphereOfflineSDK.prototype as any).renderMap === "function") {
  ...
  if (typeof (GeoSphereOfflineSDK.prototype as any).watchPosition === "function") {
  ...
  if (typeof (GeoSphereOfflineSDK.prototype as any).startTrackingSession === "function") {
  ...
  if (typeof (GeoSphereOfflineSDK.prototype as any).evaluateGeofence === "function") {
  ...
  if (typeof (GeoSphereOfflineSDK.prototype as any).calculateRoute === "function") {
  ...
  if (typeof (GeoSphereOfflineSDK.prototype as any).startTurnByTurn === "function") {
  ...
  if (typeof (GeoSphereOfflineSDK.prototype as any).searchPlaces === "function") {
  ...
  if (typeof (GeoSphereOfflineSDK.prototype as any).calculateArea === "function") {
  ...
  const readK1 = await memStorage.read<any>("k1");
  if (!readK1 || readK1.text !== "v1") throw new Error("Storage abstraction read/write failed");
  ...
  try {
    ...
    if (!(e instanceof GeoSphereOfflineError) || e.code !== "PACKAGE_NOT_FOUND") throw e;
  }

  console.log(" [40/43] Testing Sync Item Deletion / Removal...");
  const sItem = await sdk.enqueueSync("TEST_SYNC", { data: 123 });
  if (sItem.state !== "PENDING") throw new Error("Sync item creation state check failed");

  console.log(" [41/43] Testing Auto-Sync Configuration Flag...");
  if (!config.autoSync) throw new Error("autoSync config flag state check failed");

  console.log(" [42/43] Testing Conflict Resolution Strategy Flag...");
  if (config.conflictStrategy !== "SERVER_WINS") throw new Error("conflictStrategy config flag state check failed");

  console.log(" [43/43] Testing Embedded UI Configuration Mode...");
  if (!config.embeddedMode) throw new Error("embeddedMode config flag state check failed");
