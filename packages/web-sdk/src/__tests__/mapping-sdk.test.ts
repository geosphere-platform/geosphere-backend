...
import {
  GeoSphereMapping,
  GeoSphereMappingConfig,
  GeoSphereMapProvider,
  GeoSphereBasemap,
  GeoSphereMapStyle,
  MAPPING_UI_COMPONENTS,
  MAPPING_READY_MADE_SCREENS,
...
async function runMappingSdkTests() {
  ...
  const mockProvider: GeoSphereMapProvider = {
  ...
  const defaultBasemap: GeoSphereBasemap = {
    id: "osm_streets",
  ...
  const defaultStyle: GeoSphereMapStyle = {
    id: "style_modern_light",
    name: "Modern Light Preset",
    category: "light"
  };

  const mappingConfig: GeoSphereMappingConfig = {
    gisConfig: {
  ...
  const satelliteBasemap: GeoSphereBasemap = {
    id: "satellite_world",
  ...
  const darkStyle: GeoSphereMapStyle = {
    id: "style_dark_pro",
  ...
  const mappingTheme = resolveThemePrecedence({
    tenantConfig: {
      preset: "glass",
      tokens: { colors: { primary: "#6366F1", surface: "rgba(255,255,255,0.1)" } as any }
    }
  });
