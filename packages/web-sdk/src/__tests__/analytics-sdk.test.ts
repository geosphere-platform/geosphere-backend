...
import {
  ...
  GeoSphereAnalyticsConfig,
  GeoSphereMockAnalyticsProvider,
  GeoSphereAnalyticsError,
  GeoSphereAnalyticsEvent,
  ANALYTICS_UI_COMPONENTS,
  ANALYTICS_READY_MADE_SCREENS,
...
async function runAnalyticsSdkTests() {
  ...
  const config: GeoSphereAnalyticsConfig = {
    embeddedMode: true,
    batchSize: 20,
  ...
  if (sdk.getConsent() !== "DENIED") throw new Error("setConsent failed to update consent state");
  ...
  const searchTelemetry: GeoSphereAnalyticsEvent = {
  ...
  if (rtMetric.name !== "realtime.reconnect.count") throw new Error("Realtime metric boundary check failed");
  ...
  if (notifMetric.name !== "notifications.delivery.failures") throw new Error("Notifications metric boundary check failed");
  ...
  if (!theme.colors.primary) throw new Error("Theme resolution failed for Analytics SDK");
  ...
  if (viewState.status !== "granted") throw new Error("RBAC evaluation failed for granted permission");
  ...
  if (typeof (GeoSphereAnalyticsSDK.prototype as any).createPolygon === "function") {
  ...
  if (typeof (GeoSphereAnalyticsSDK.prototype as any).renderMap === "function") {
  ...
  if (typeof (GeoSphereAnalyticsSDK.prototype as any).watchPosition === "function") {
  ...
  if (typeof (GeoSphereAnalyticsSDK.prototype as any).startTrackingSession === "function") {
  ...
  if (typeof (GeoSphereAnalyticsSDK.prototype as any).evaluateGeofence === "function") {
  ...
  if (typeof (GeoSphereAnalyticsSDK.prototype as any).calculateRoute === "function") {
  ...
  if (typeof (GeoSphereAnalyticsSDK.prototype as any).startTurnByTurn === "function") {
  ...
  if (typeof (GeoSphereAnalyticsSDK.prototype as any).searchPlaces === "function") {
  ...
  if (typeof (GeoSphereAnalyticsSDK.prototype as any).calculateArea === "function") {
  ...
  if (typeof (GeoSphereAnalyticsSDK.prototype as any).createMapPackage === "function") {
  ...
  if (typeof (GeoSphereAnalyticsSDK.prototype as any).connectWebSocket === "function") {
  ...
  if (typeof (GeoSphereAnalyticsSDK.prototype as any).generateEmployeeReport === "function") {
  ...
  if (typeof (GeoSphereAnalyticsSDK.prototype as any).generateSalesMetrics === "function") {
  ...
  if (config.batchSize !== 20) throw new Error("batchSize config check failed");

  console.log(" [38/43] Testing Config Flush Interval Defaults...");
  if (config.flushIntervalMs !== 5000) throw new Error("flushIntervalMs config check failed");
  ...
  if (updatedPrivacy.performanceEnabled !== false) throw new Error("updatePrivacyConfig failed to update flag");

  console.log(" [41/43] Testing Multi-Platform Web Adapter Metadata...");
  if (!dashScreen.adapters || dashScreen.adapters.length !== 3) throw new Error("dashScreen adapters check failed");
  ...
  if (!androidAdapter || androidAdapter.framework !== "compose") throw new Error("Android Compose adapter check failed");

  console.log(" [43/43] Testing Multi-Platform iOS Adapter Metadata...");
  const iosAdapter = dashScreen.adapters.find((a) => a.platform === "ios");
  if (!iosAdapter || iosAdapter.framework !== "swiftui") throw new Error("iOS SwiftUI adapter check failed");
