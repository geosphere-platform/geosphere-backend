...
import {
  ...
  GeoSphereModuleContract,
  GeoSphereComponentDefinition,
  GeoSphereScreenDefinition
...
async function runContractTests() {
  ...
  const mockTrackingModule: GeoSphereModuleContract = {
    id: "tracking",
    version: "1.0.0",
  ...
  const registeredModules: Map<string, GeoSphereModuleContract> = new Map();
  ...
  const unsubscribe = eventBus.on("themeChanged", (event: GeoSphereEvent<{ mode: string }>) => {
  ...
  const resolved = resolveThemePrecedence({
    ...
    tenantConfig: {
      tokens: { colors: { primary: "#7C3AED" } as any }
    },
    userPreference: {
      tokens: { colors: { primary: "#10B981" } as any }
    }
  });
  ...
  const vehicleCardDef: GeoSphereComponentDefinition = {
  ...
  const trackingScreenDef: GeoSphereScreenDefinition = {
