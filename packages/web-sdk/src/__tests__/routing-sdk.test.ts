...
import {
  GeoSphereRoutingSDK,
  GeoSphereRoutingConfig,
  GeoSphereRouteRequest,
  GeoSphereMockRoutingProvider,
  validateCoordinate,
  normalizeWaypoint,
  GeoSphereRoutingError,
  ROUTING_UI_COMPONENTS,
  ROUTING_READY_MADE_SCREENS,
...
async function runRoutingSdkTests() {
  ...
  const config: GeoSphereRoutingConfig = {
  ...
  try {
    ...
    if (!e.message.includes("INVALID_COORDINATE")) throw e;
  }
  ...
  if (!info.name) throw new Error("getProviderInfo failed");
  ...
  if ((caps as any).includes("TRAFFIC") || (caps as any).includes("ISOCHRONES")) {
  ...
  const waypointsReq: GeoSphereRouteRequest = {
  ...
  if (typeof (routingSdk as any).render === "function" || typeof (routingSdk as any).componentDidMount === "function") {
  ...
  const hasNavSession = typeof (GeoSphereRoutingSDK.prototype as any).startNavigation === "function";
  if (hasNavSession) {
  ...
  const hasOwnPoly = typeof (GeoSphereRoutingSDK.prototype as any).computeIntersection === "function";
  if (hasOwnPoly) {
  ...
  const hasOwnGPS = typeof (GeoSphereRoutingSDK.prototype as any).getCurrentPosition === "function";
  if (hasOwnGPS) {
