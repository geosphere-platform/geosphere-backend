...
import {
  GeoSphereNavigationSDK,
  GeoSphereNavigationConfig,
  GeoSphereRoutingSDK,
  GeoSphereRouteResponse,
  NAVIGATION_UI_COMPONENTS,
  NAVIGATION_READY_MADE_SCREENS,
...
async function runNavigationSdkTests() {
  ...
  const config: GeoSphereNavigationConfig = {
    autoReroute: true,
  ...
  if ((caps as any).includes("BACKGROUND_NAVIGATION") || (caps as any).includes("VOICE_NAVIGATION") || (caps as any).includes("OFFLINE_NAVIGATION")) {
  ...
  const routeResponse: GeoSphereRouteResponse = await routingSdk.calculateRoute({
  ...
  if (typeof (navigationSdk as any).render === "function" || typeof (navigationSdk as any).componentDidMount === "function") {
  ...
  const hasVoice = typeof (GeoSphereNavigationSDK.prototype as any).speakManeuver === "function";
  if (hasVoice) {
  ...
  const hasOwnRouting = typeof (GeoSphereNavigationSDK.prototype as any).buildRoutingGraph === "function";
  if (hasOwnRouting) {
  ...
  const hasOwnGPS = typeof (GeoSphereNavigationSDK.prototype as any).getCurrentPosition === "function";
  if (hasOwnGPS) {
  ...
  const hasOwnPoly = typeof (GeoSphereNavigationSDK.prototype as any).computeIntersection === "function";
  if (hasOwnPoly) {
  ...
  const hasOwnFence = typeof (GeoSphereNavigationSDK.prototype as any).checkPointInGeofence === "function";
  if (hasOwnFence) {
  ...
  const hasOfflineTiles = typeof (GeoSphereNavigationSDK.prototype as any).downloadMapTiles === "function";
  if (hasOfflineTiles) {
