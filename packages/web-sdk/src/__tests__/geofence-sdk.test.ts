...
import {
  GeoSphereGeofencingSDK,
  GeoSphereGeofenceConfig,
  GeoSphereGeofence,
  validateGeofenceGeometry,
  pointInCircle,
  pointInPolygon,
  evaluateGeofenceState,
  GEOFENCE_UI_COMPONENTS,
  GEOFENCE_READY_MADE_SCREENS,
...
async function runGeofenceSdkTests() {
  ...
  const config: GeoSphereGeofenceConfig = {
  ...
  const circleGeofence: GeoSphereGeofence = {
    id: "zone_hq_circle",
  ...
  const circleGeom = circleGeofence.geometry as { type: "circle"; center: [number, number]; radiusMeters: number };
  ...
  const polyGeom = polygonGeofence.geometry as { type: "polygon"; coordinates: [number, number][][] };
  ...
  const overlapCircle: GeoSphereGeofence = {
    id: "zone_city_broad",
  ...
  if ((caps as any).includes("BACKGROUND_GEOFENCING")) {
  ...
  if (typeof (geofenceSdk as any).render === "function" || typeof (geofenceSdk as any).componentDidMount === "function") {
  ...
  const hasOwnPoly = typeof (GeoSphereGeofencingSDK.prototype as any).computeIntersection === "function";
  if (hasOwnPoly) {
  ...
  const hasOwnGPS = typeof (GeoSphereGeofencingSDK.prototype as any).getCurrentPosition === "function";
  if (hasOwnGPS) {
