...
import {
  ...
  GeoSphereWorkflowConfig,
  GeoSphereMockWorkflowProvider,
  GeoSphereWorkflowError,
  WORKFLOW_UI_COMPONENTS,
  WORKFLOW_READY_MADE_SCREENS,
...
async function runWorkflowSdkTests() {
  ...
  const config: GeoSphereWorkflowConfig = {
    embeddedMode: true
  };
  ...
  try {
    ...
    if (!(err instanceof GeoSphereWorkflowError) || err.code !== "TRANSITION_INVALID") throw err;
  }
  ...
  if (!theme.colors.primary) throw new Error("Theme resolution failed for Workflow SDK");
  ...
  if (execState.status !== "granted") throw new Error("RBAC evaluation failed for granted permission");

  const adminState = evaluatePermissionState(userPerms, "workflows.admin", "disable");
  if (adminState.status !== "denied" || adminState.mode !== "disable") throw new Error("RBAC evaluation failed for denied permission");
  ...
  if (typeof (GeoSphereWorkflowSDK.prototype as any).createPolygon === "function") {
  ...
  if (typeof (GeoSphereWorkflowSDK.prototype as any).renderMap === "function") {
  ...
  if (typeof (GeoSphereWorkflowSDK.prototype as any).watchPosition === "function") {
  ...
  if (typeof (GeoSphereWorkflowSDK.prototype as any).startTrackingSession === "function") {
  ...
  if (typeof (GeoSphereWorkflowSDK.prototype as any).evaluateGeofence === "function") {
  ...
  if (typeof (GeoSphereWorkflowSDK.prototype as any).calculateRoute === "function") {
  ...
  if (typeof (GeoSphereWorkflowSDK.prototype as any).startTurnByTurn === "function") {
  ...
  if (typeof (GeoSphereWorkflowSDK.prototype as any).searchPlaces === "function") {
  ...
  if (typeof (GeoSphereWorkflowSDK.prototype as any).calculateArea === "function") {
  ...
  if (typeof (GeoSphereWorkflowSDK.prototype as any).createMapPackage === "function") {
  ...
  if (typeof (GeoSphereWorkflowSDK.prototype as any).createEmployeeWorkflow === "function") {
  ...
  if (typeof (GeoSphereWorkflowSDK.prototype as any).approveVisit === "function") {
  ...
  try {
    ...
    if (!(e instanceof GeoSphereWorkflowError) || e.code !== "DEFINITION_INVALID") throw e;
  }
  ...
  if (rejectedAppr.status !== "REJECTED") throw new Error("submitApproval rejection failed");

  console.log(" [38/42] Testing Optimistic Concurrency Version Number Increment...");
  if (finalExec.versionNumber !== 3) throw new Error(`versionNumber check failed: expected 3, got ${finalExec.versionNumber}`);

  console.log(" [39/42] Testing Multi-Platform Web Adapter Metadata...");
  if (!execScreen.adapters || execScreen.adapters.length !== 3) throw new Error("execScreen adapters check failed");
  ...
  const tenantAWorkflow: string = "wf_tenant_A";
  const tenantBWorkflow: string = "wf_tenant_B";
  if (tenantAWorkflow === tenantBWorkflow) throw new Error("Tenant boundary check failed");
