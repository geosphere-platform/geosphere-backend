...
export class GeoSphereTaskKanbanController {
  private tasks: SDKKanbanTask[];

  constructor(tasks: SDKKanbanTask[]) {
    this.tasks = tasks;
  }

  public getTasksByStatus(status: SDKKanbanTask["status"]): SDKKanbanTask[] {
    return this.tasks.filter((t) => t.status === status);
  }

  public transitionTaskStatus(taskId: string, newStatus: SDKKanbanTask["status"]): SDKKanbanTask | null {
  ...
  public renderHTML(): string {
    const statuses: SDKKanbanTask["status"][] = ["ASSIGNED", "ACCEPTED", "IN_PROGRESS", "COMPLETED"];
