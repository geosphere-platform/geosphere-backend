...
export class GeoSphereMapComponent {
  public renderHTML(mapId: string = "geosphere-map-canvas"): string {
...
export class GeoSphereLayerManagerComponent {
  public renderHTML(layers: Array<{ id: string; name: string; visible: boolean; opacity?: number }>): string {
    const theme = GeoSphereThemeManager.getTheme();
    return `<div class="geosphere-card p-4 space-y-3 font-sans">
      ...
      <div class="space-y-2 max-h-48 overflow-y-auto">
        ${layers
          .map(
            (lyr) => `<div class="flex items-center justify-between p-2 rounded bg-slate-900 border border-slate-800 text-xs">
...
export class GeoSphereBaseMapSelectorComponent {
  public renderHTML(activeBasemap: string): string {
    const basemaps = [
    ...
    return `<div class="geosphere-card p-3 space-y-2 font-mono text-xs">
      ...
      <div class="grid grid-cols-3 gap-1.5">
        ${basemaps
          .map(
            (bm) => `<button class="px-2 py-1.5 rounded font-bold transition-all text-center ${
              activeBasemap === bm.id
...
export class GeoSphereJurisdictionSelectorComponent {
  public renderHTML(jurisdictions: Array<{ id: string; name: string; level: string; selected?: boolean }>): string {
    return `<div class="geosphere-card p-4 space-y-3 font-sans text-xs">
      ...
      <div class="space-y-1.5 max-h-40 overflow-y-auto">
        ${jurisdictions
          .map(
            (j) => `<div class="flex justify-between items-center p-2 rounded bg-slate-900/60 border border-slate-800">
...
export class GeoSphereFeatureDetailsComponent {
  public renderHTML(feature: { id: string; title: string; properties: Record<string, any> }): string {
    return `<div class="geosphere-card p-4 space-y-3 font-sans text-xs">
      <div class="flex justify-between items-center border-b border-slate-800 pb-2">
      ...
      <div class="space-y-1.5 font-mono text-[11px]">
        ${Object.entries(feature.properties)
          .map(
            ([k, v]) => `<div class="flex justify-between p-1.5 rounded bg-slate-900 border border-slate-800">
...
export class GeoSphereLegendComponent {
  public renderHTML(items: Array<{ label: string; color: string }>): string {
    return `<div class="geosphere-card p-3 space-y-2 font-mono text-xs">
      <span class="text-slate-400 font-bold text-[11px] block border-b border-slate-800 pb-1">MAP LEGEND</span>
      <div class="space-y-1">
        ${items
          .map(
            (item) => `<div class="flex items-center gap-2 text-slate-300">
